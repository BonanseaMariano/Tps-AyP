...
void agregarDNI(Persona *persona);
...
int main() {
    ...
    archivo = fopen("datosImportadosConDNI.dat", "r+");
...
void agregarDNI(Persona *persona){
    ...
    int orden = 1000;
    //Abro archivo viejo
    archivoViejo = fopen("datosImportadosConOrden.dat","r");
    //Abro archivo nuevo con numeros de orden
    archivo = fopen("datosImportadosConDNI.dat", "w");
    ...
    if (!fread(&personaAntigua, sizeof(PersonaAntigua), 1, archivoViejo))
    {
        ...
        while (fread(&personaAntigua, sizeof(PersonaAntigua), 1, archivoViejo)) {
            ...
            persona->numOrden = personaAntigua.numOrden;
            printf("Ingrese el DNI de \"%s %s\":",persona->apellido,persona->nombre);
            scanf("%s",&persona->dni);
...
void agregarPersona(FILE *archivo, Persona *persona) {
    ...
    printf("Ingrese el numero de DNI de la persona: ");
    scanf("%s", persona->dni);
...
int buscarOrden(FILE *archivo, Persona *persona) {
    char dniBusq[50];
    int i=0; //Contador para la lista de personas
    rewind(archivo);//Reinicio el cursor del archivo

    printf("Ingrese el numero de DNI de la persona que desea buscar: ");
    scanf("%s", dniBusq);
    printf("\n");

    while (fread(persona, sizeof(Persona), 1, archivo)) {
        if (strcmp(persona->dni, dniBusq) == 0) { //Chiteo buscando por dni pero paso el numero de orden de la persona para las otras funciones
            printf("\t------- %s* DNI ------\n",persona->dni);
            printf("Numero de Orden: %d\n", persona->numOrden);
            printf("Nombre: %s\n", persona->nombre);
            printf("Apellido: %s\n", persona->apellido);
    ...
    printf("\tX No se encontro ningun persona con el DNI ingresado X\n");
    return -1;
}
...
void eliminarOrden(FILE *archivo, Persona *persona){
    ...
    if (ordSM!=-1)//Esto significa que en la funcion de buscar si encontro el numero de orden
    {
        ...
        fclose(archivo);
        remove("datosImportadosConDNI.dat");
        
        //Cierro el archivo temporal y lo renombro
        fclose(archivoMOD);
        if(rename("temp.dat", "datosImportadosConDNI.dat") != 0){
